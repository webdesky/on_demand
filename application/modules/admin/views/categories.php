                ...
                <div class="panel-body">
                    ...
                    <?php 
                        if(!empty($categories[0])){
                            $url = base_url('admin/category/'.$categories[0]->id);
                        }else{
                        ...
                        <div class="col-lg-12 col-md-12">
                            <form role="form" method="post" action="<?php echo $url; ?>" class="registration_form1" enctype="multipart/form-data">
                                <div class="form-group"> <label class="col-md-2">Category Name *</label>
                                    <div class="col-lg-6"> <input class="form-control" type="text" placeholder="Category name" name="name" autocomplete="off" id="name" required="required" value="<?php if(!empty($categories[0]->name)){ echo $categories[0]->name;}else{ echo set_value('name');} ?>">
                                    <span class="red" id="old">
                                            <?php echo form_error('name'); ?></span> </div>
                                </div>
                              
                                <div class="form-group"> <label class="col-md-2">Short Description </label>
                                    <div class="col-lg-6"> <textarea name="description" id="description" class="form-control" placeholder="Category Short Description"><?php if(!empty($categories[0]->description)){ echo $categories[0]->description;}else{ echo set_value('description');} ?></textarea><span class="red" id="new">
                                            <?php echo form_error('description'); ?></span> </div>
                                </div>
                                <div class="form-group"> <label class="col-md-2">Images </label>
                                    <div class="col-lg-6"> <input name="images[]" class="form-control" type="file">
                                        <?php   
                                        if(!empty($categories[0]->image)){
                                            $image = explode(',',$categories[0]->image); 
